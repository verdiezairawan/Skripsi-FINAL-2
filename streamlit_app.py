...
def get_coingecko_data(days=365): # -> Nilai default diubah menjadi 365
    ...
    def make_request_with_retry(url, max_retries=3, delay=2):
        """Make request with retry mechanism for rate limiting"""
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Request failed, retrying in {delay}s... Error: {e}")
                    import time
                    time.sleep(delay)
                    delay *= 2
                    continue
    ...
    try:
        # Coba ambil data OHLC terlebih dahulu (lebih akurat)
        try:
            if days <= 7:
                # Untuk 7 hari atau kurang, gunakan data per jam (lebih akurat)
                url = f"https://api.coingecko.com/api/v3/coins/bitcoin/ohlc?vs_currency=usd&days={days}"
                response = make_request_with_retry(url)
                data = response.json()
                
                # Parse OHLC data (format: [timestamp, open, high, low, close])
                ...
                # Resample ke daily data jika diperlukan
                if days > 1:
                ...
                # Tambahkan volume (gunakan market_chart untuk volume)
                volume_url = f"https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days={days}"
                volume_response = make_request_with_retry(volume_url)
                volume_data = volume_response.json()
                
                volumes_df = pd.DataFrame(volume_data['total_volumes'], columns=['timestamp', 'volume'])
                ...
                # Merge dengan data OHLC - gunakan left join untuk mempertahankan semua data OHLC
                df_final = pd.merge(df_daily, volumes_df, left_index=True, right_index=True, how='left')
                
                # Jika volume kosong, isi dengan 0
                df_final['volume'] = df_final['volume'].fillna(0)
                
                logger.info(f"Data OHLC shape: {df_daily.shape}")
                logger.info(f"Volume shape: {volumes_df.shape}")
                logger.info(f"Final shape after merge: {df_final.shape}")
                
            else:
                # Untuk periode lebih dari 7 hari, gunakan kombinasi OHLC + market_chart
                # Ambil data per jam untuk 7 hari terakhir
                ohlc_url = "https://api.coingecko.com/api/v3/coins/bitcoin/ohlc?vs_currency=usd&days=7"
                ohlc_response = make_request_with_retry(ohlc_url)
                ohlc_data = ohlc_response.json()
                
                # Parse OHLC data 7 hari terakhir
                ohlc_recent = pd.DataFrame(ohlc_data, columns=['timestamp', 'open', 'high', 'low', 'close'])
                ohlc_recent['Date'] = pd.to_datetime(ohlc_recent['timestamp'], unit='ms')
                ohlc_recent = ohlc_recent.set_index('Date')
                ohlc_recent = ohlc_recent.drop('timestamp', axis=1)
                
                # Resample ke daily untuk 7 hari terakhir
                ohlc_recent_daily = ohlc_recent.resample('D').agg({
                    'open': 'first',
                    'high': 'max',
                    'low': 'min',
                    'close': 'last'
                ...
                # Ambil data historis untuk periode yang lebih lama
                market_url = f"https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days={days}"
                market_response = make_request_with_retry(market_url)
                market_data = market_response.json()
                
                # Parse prices data historis
                ...
                # Resample ke daily dan hitung OHLCV dari data historis
                prices_daily = prices_df.resample('D').agg({
                    'price': ['first', 'max', 'min', 'last']
                })
                prices_daily.columns = ['open', 'high', 'low', 'close']
                
                # Gabungkan data recent (OHLC) dengan data historis
                cutoff_date = ohlc_recent_daily.index[0]
                df_historical = prices_daily[prices_daily.index < cutoff_date]
                df_final = pd.concat([df_historical, ohlc_recent_daily])
                ...
                # Merge dengan data OHLC
                df_final = pd.merge(df_final, volumes_df, left_index=True, right_index=True, how='outer')
            
            logger.info("✅ Berhasil menggunakan data OHLC dari CoinGecko")
            
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 429:
                logger.warning("⚠️ Rate limit tercapai, menggunakan fallback market_chart...")
                # Fallback ke market_chart API jika OHLC gagal
                raise e  # Re-raise untuk masuk ke fallback
            else:
                raise e
                
    except (requests.exceptions.RequestException, requests.exceptions.HTTPError) as e:
        # Fallback: Gunakan market_chart API dengan interval yang lebih baik
        logger.info("🔄 Menggunakan fallback market_chart API...")
        
        try:
            # Gunakan market_chart API dengan interval yang lebih baik
            url = f"https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=usd&days={days}"
            response = make_request_with_retry(url)
            data = response.json()
            
            # Parse prices data dengan interval yang lebih baik
            ...
            # Resample ke daily data dengan interval yang lebih baik
            # Gunakan interval yang lebih kecil untuk mendapatkan variasi yang lebih baik
            if days <= 7:
                # Untuk 7 hari, resample setiap 4 jam untuk mendapatkan variasi
                df_4h = df_combined.resample('4H').agg({
                    'price': ['first', 'max', 'min', 'last'],
                    'volume': 'sum'
                })
                df_4h.columns = ['open', 'high', 'low', 'close', 'volume']
                
                # Resample lagi ke daily
                ...
                # Untuk 30 hari, resample setiap 6 jam
                df_6h = df_combined.resample('6H').agg({
                    'price': ['first', 'max', 'min', 'last'],
                    'volume': 'sum'
                })
                df_6h.columns = ['open', 'high', 'low', 'close', 'volume']
                
                df_daily = df_6h.resample('D').agg({
                    'open': 'first',
                    'high': 'max',
                    'low': 'min',
                    'close': 'last',
                ...
                # Untuk periode lebih lama, gunakan interval yang lebih besar
            ...
            logger.info("✅ Berhasil menggunakan fallback market_chart API")
            
        except Exception as fallback_error:
            error_message = f"Fallback juga gagal: {fallback_error}"
    ...
    # Debug info
    logger.info(f"Data berhasil diambil: {len(df_final)} baris")
    if len(df_final) > 0:
        logger.info(f"Range tanggal: {df_final.index[0]} hingga {df_final.index[-1]}")
        logger.info(f"Sample data:\n{df_final.head()}")
        
        # Verifikasi bahwa nilai OHLC berbeda
        sample_row = df_final.iloc[0]
        if sample_row['Open'] == sample_row['High'] == sample_row['Low'] == sample_row['Close']:
            logger.warning("⚠️ Nilai OHLC sama semua, kemungkinan ada masalah dengan data")
        else:
            logger.info("✅ Nilai OHLC bervariasi (normal)")
    else:
        logger.warning("Data kosong setelah processing")
